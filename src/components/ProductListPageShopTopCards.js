...
export default function ProductListPageShopTopCards() {
  ...
  const dropDownHandler = (data) => {
    ...
    api
      .get(
        `products?filter=${data.gender === "k" ? "kadın" : "erkek"}&category=${
          data.id
        }`
      )
      .then((response) => {
  ...
  return (
    <div className="w-[70rem] mobile:flex-wrap mobile:w-[20rem] mx-auto flex mt-8 gap-4">
      {sortedProductList.map((eleman, index) => (
        <NavLink
          onClick={() => dropDownHandler(eleman)}
          key={index}
          to={`/products/${eleman.gender === "k" ? "kadın" : "erkek"}/${
            eleman.title
          }`}
        >
          <div
            key={index}
            className="w-[12rem]  mobile:min-w-max mobile:my-4 mobile:mx-auto h-[13rem] "
          >
